...
export default async function AdminHeroPage() {
  ...
  const [{ data: slidesData }, { data: statsData }, { data: settingsData }] = await Promise.all([
    supabase
      .from("hero_slides")
  ...
  const heroSettings = {
    hero_mode: (settingsData?.hero_mode ?? "slides") as "slides" | "video",
    video_url: settingsData?.video_url ?? "",
    video_thumb_url: settingsData?.video_thumb_url ?? "",
  };
    ...
    <div className="px-4 py-6 md:px-8 md:py-10 max-w-4xl">
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm">
          {slides.filter(s => s.is_active).length} active slides · {stats.filter(s => s.is_active).length} active stats · mode: <span className="text-accent font-semibold">{heroSettings.hero_mode}</span>
        </p>
      </div>
      <HeroManager slides={slides} stats={stats} heroSettings={heroSettings} />
    </div>
  );
}
